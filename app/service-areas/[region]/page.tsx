...
const advice:Record<string,string>={
  ...
  "Brisbane Southside":"When water appears after rain, note the wind direction if known and whether it follows heavy or light showers. Roof, flashing and gutter checks should follow where and when water appears rather than assuming the ceiling mark is the entry point.",
...
const quoteQuestions:Record<string,{scenario:string;checks:string[];scope:string}>={
  ...
  "Brisbane Northside":{
    scenario:"For a tiled-roof enquiry, a broken tile, ridge, valley and gutter fault call for different materials and checks; describe where water appears or what damage you can see before requesting restoration.",
...
export default async function RegionPage({params}:{params:Promise<{region:string}>}){
  ...
  return <PageShell><PageHero eyebrow="BRISBANE ROOF SERVICE AREA" title={`Roof repair enquiries in ${area.name}`} description={advice[area.name]} image={regionHeroImages[area.name]}/>
    <section className="section"><div className="shell"><p><Link href="/service-areas">← All Brisbane service areas</Link></p><SectionHeading eyebrow="POPULAR SUBURBS" title={`Find your ${area.name} suburb`} copy="Give the property suburb, when any leak appears and the roof material if known. Inspection confirms the fault, materials and safe access."/><div className="area-card-grid"><article><ul className="suburb-list">{area.suburbs.map(suburb=><li id={slug(suburb)} key={suburb}><Link href={`/contact?suburb=${encodeURIComponent(suburb)}`}>{suburb} →</Link></li>)}</ul></article></div></div></section>
